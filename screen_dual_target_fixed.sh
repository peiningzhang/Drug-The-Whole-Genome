#!/bin/bash
# DrugCLIP 双目标筛选脚本（修复版）
# 筛选能同时匹配两个口袋的分子（取交集）
# 使用方法: bash screen_dual_target_fixed.sh <target_dir> [gpu_id] [top_n]

set -e

if [ $# -lt 1 ]; then
    echo "使用方法: bash screen_dual_target_fixed.sh <target_dir> [gpu_id] [top_n]"
    echo ""
    echo "参数说明:"
    ...
    echo ""
    echo "示例:"
    echo "  bash screen_dual_target_fixed.sh ./data/targets/KOR-5H2A 0 1000"
    exit 1
fi
...
for pdb_file in "${PDB_FILES[@]}"; do
    ...
    # 编码口袋
    echo "步骤 1: 从 PDB 文件生成 pocket.lmdb..."
    ...
    CUDA_VISIBLE_DEVICES="${GPU_ID}" python ./unimol/retrieval.py \
        ...
        --use-cache True \
        --save-path ${result_file} 2>&1 | tail -20
...
def get_intersection_with_scores(all_molecules, intersection_smiles):
    ...
    for smiles in intersection_smiles:
        ...
        result.append((smiles, avg_score))
...
if __name__ == "__main__":
    if len(sys.argv) < 4:
        print("使用方法: python compute_intersection.py <top_n> <output_file> <result_file1> <result_file2> ...")
        sys.exit(1)
    ...
    with open(output_file, 'w') as f:
        f.write("SMILES,avg_score\n")
        for smiles, avg_score in intersection_with_scores:
            f.write(f"{smiles},{avg_score:.6f}\n")
    
    print(f"\n完成！共 {len(intersection_with_scores)} 个共同分子")
    print(f"\n前 10 个共同分子:")
    for i, (smiles, score) in enumerate(intersection_with_scores[:10]):
        print(f"  {i+1}. {smiles}: {score:.6f}")
PYTHON_EOF
...
python ${python_script} ${TOP_N} ${intersection_file} "${RESULTS[@]}"
